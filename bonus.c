...
#define MAX     1000
#define SPLIT   16
#define EXTRA_THREAD_NUMBER 7

int thread_number = EXTRA_THREAD_NUMBER;

pthread_t thread[EXTRA_THREAD_NUMBER];
...
sema semaphore[EXTRA_THREAD_NUMBER];
...
int pthread_sema_value(sema *s) {
    return s->value;
}

void pthread_sema_wait(sema *s)
{
...
void *merge_sort(void *addr) {
    ...
    if (block->size > SPLIT) {
        ...
        int thread_position = -1;

        for (int i = 0; i < thread_number; i++) {
            if (pthread_sema_value(&semaphore[i]) > 0) {
        ...
        if (thread_available) {
            pthread_sema_wait(&semaphore[thread_position]);

            pthread_create(&thread[thread_position], NULL, &merge_sort, &right_block);
            ...
            pthread_sema_post(&semaphore[thread_position]); 
        } else {
            merge_sort(&left_block);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < thread_number; i++) {
        pthread_sema_init(&semaphore[i], 1);
    }
    ...
    pthread_create(&thread, NULL, merge_sort, &left_block);
