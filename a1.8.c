...
int main(int argc, char *argv[]) {
    ...
    block.data = (int *) mmap(NULL, sizeof(int)*block.size, PROT_READ | PROT_WRITE,  MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    if (block.data == NULL) {
    ...
    two_process_merge_sort(block_two);
    two_process_merge_sort(block_three);
    two_process_merge_sort(block_four);
    two_process_merge_sort(block_five);
    two_process_merge_sort(block_six);
    two_process_merge_sort(block_seven);
    two_process_merge_sort(block_eight);
...
void two_process_merge_sort(struct block block) {
    ...
    left_block.data = block.data;
    right_block.size = block.size - left_block.size; // left_block.size + (block->size % 2);
    right_block.data = (int *) mmap(NULL, sizeof(int)*block.size, PROT_READ | PROT_WRITE,  MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    right_block.data = block.data + left_block.size;
