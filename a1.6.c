...
void two_process_merge_sort(struct block block) {
    ...
    left_block.data = block.data;
    right_block.size = block.size - left_block.size; // left_block.size + (block->size % 2);
    right_block.data = (int *)calloc(block.size, sizeof(int));
    right_block.data = block.data + left_block.size;

    int my_pipe[2];
    int *read_fd = &my_pipe[0];
    int *write_fd = &my_pipe[1];
    ...
    if (cpid != 0) {
        ...
        for (int i = 0; i < right_block.size; i++) {
            read(*read_fd, &right_block.data[i], sizeof(int));
        }
        ...
        for (int i = 0; i < right_block.size; i++) {
            write(*write_fd, &right_block.data[i], sizeof(int));
        }
